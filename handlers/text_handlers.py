...
from handlers.menu import get_menu_keyboard, get_back_reset_keyboard

async def all_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_data = context.user_data
    msg = update.message.text.strip().lower()

    # Luôn cho phép gọi nhanh menu/reset/hướng dẫn
    if msg in ["/menu", "menu", "quay lại", "⬅️ quay lại menu", "trở lại"]:
        user_data.clear()
        await update.message.reply_text("📋 Chọn chức năng:", reply_markup=get_menu_keyboard())
        return

    if msg in ["/reset", "reset", "🔄 reset"]:
        user_data.clear()
        await update.message.reply_text("🔄 Đã reset trạng thái. Bạn có thể bắt đầu lại bằng lệnh /menu hoặc chọn lại chức năng!", reply_markup=get_menu_keyboard())
        return

    if msg in ["hướng dẫn", "huong dan", "ℹ️ hướng dẫn", "/help", "help"]:
    ...
    if 'wait_for_xien_input' in user_data:
        ...
        if not xiens:
            await update.message.reply_text("⚠️ Không ghép được xiên, vui lòng nhập lại.", reply_markup=get_back_reset_keyboard())
        else:
            reply = f"{len(xiens)} bộ xiên {do_dai}:\n" + ', '.join(xiens[:50])
            await update.message.reply_text(reply, reply_markup=get_back_reset_keyboard())
        user_data.clear()
        return
    ...
    if user_data.get("wait_cang4d_numbers"):
        ...
        if not arr or not all(len(n) == 3 for n in arr):
            await update.message.reply_text("⚠️ Mỗi số cần đúng 3 chữ số! (VD: 123 456 789)", reply_markup=get_back_reset_keyboard())
            return
    ...
    if user_data.get('wait_phongthuy_ngay_duong'):
        ...
        try:
            ...
            await update.message.reply_text("❗️ Nhập ngày không hợp lệ! Dùng dạng YYYY-MM-DD hoặc DD-MM, ví dụ: 2024-07-22 hoặc 22-07.", reply_markup=get_back_reset_keyboard())
    ...
    if user_data.get('wait_phongthuy_ngay_canchi'):
        ...
        if sohap_info is None:
            await update.message.reply_text("❗️ Không tìm thấy thông tin can chi hoặc số hạp với tên bạn nhập! Kiểm tra lại định dạng (VD: Giáp Tý).", reply_markup=get_back_reset_keyboard())
        else:
            text = phong_thuy_format(can_chi, sohap_info)
            await update.message.reply_text(text, parse_mode="Markdown", reply_markup=get_back_reset_keyboard())
