...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.effective_message.reply_text("✨ Chào mừng bạn đến với bot Xổ Số Telegram! Sử dụng lệnh /menu để bắt đầu.")
...
async def menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.reply_text("📋 Vui lòng chọn chức năng bên dưới:", reply_markup=reply_markup)

async def kqxs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    result = get_kqxs_mienbac()
    if "error" in result:
        await update.effective_message.reply_text(f"❌ Lỗi khi lấy kết quả: {result['error']}")
        return
    reply = ""
    ...
    await update.effective_message.reply_text("👉 Bạn muốn làm gì tiếp?:", reply_markup=InlineKeyboardMarkup(keyboard))

# Luồng xử lý ghép càng
async def ghepcang_popup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    user_gh_cang[user_id] = {}
    await update.effective_message.reply_text("🔢 Nhập loại ghép càng (3D hoặc 4D):", reply_markup=ReplyKeyboardRemove())
    return GH_CANG_TYPE
...
async def ghepcang_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if kieu not in ["3D", "4D"]:
        await update.effective_message.reply_text("⚠️ Chỉ chấp nhận 3D hoặc 4D. Vui lòng nhập lại:")
        return GH_CANG_TYPE
    user_gh_cang[user_id]["kieu"] = kieu
    await update.effective_message.reply_text("✏️ Nhập danh sách số càng (VD: 3 4):")
    return GH_CANG_LIST
...
async def ghepcang_cang(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.reply_text("✏️ Nhập các số để ghép (VD: 123 456):")
    return GH_SO_LIST

async def ghepcang_so(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    numbers = [x.zfill(3) for x in update.message.text.replace(',', ' ').split() if x.isdigit()]
    data = user_gh_cang.get(user_id, {})
    if not numbers or "kieu" not in data or "cangs" not in data:
        await update.effective_message.reply_text("❌ Dữ liệu bị thiếu. Gõ lại từ đầu.")
    ...
    if not results:
        await update.effective_message.reply_text("❌ Không có kết quả nào phù hợp.")
    else:
        await update.effective_message.reply_text(', '.join(results))
        keyboard = [[InlineKeyboardButton("⬅️ ⬅️ Trở về menu chính", callback_data="back_to_menu")]]
        await update.effective_message.reply_text("👉 Bạn muốn làm gì tiếp?:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def bat_tudong(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.reply_text("✅ Đã bật gửi ảnh kết quả lúc 18:40 mỗi ngày.")

async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.effective_message.reply_text("⛔️ Đã hủy bỏ thao tác hiện tại.")
    keyboard = [[InlineKeyboardButton("⬅️ ⬅️ Trở về menu chính", callback_data="back_to_menu")]]
    await update.effective_message.reply_text("⬅️ Trở về menu chính:", reply_markup=InlineKeyboardMarkup(keyboard))
    return ConversationHandler.END
...
async def ghepxien_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.reply_text("🔢 Nhập các số muốn ghép (VD: 22 33 44):")
    return XIEN_SO_LIST
...
async def ghepxien_sos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.reply_text("🔢 Nhập kiểu xiên (2, 3 hoặc 4):")
    return XIEN_KIEU
...
async def ghepxien_kieu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.effective_message.reply_text("⚠️ Kiểu xiên không hợp lệ. Nhập số 2, 3 hoặc 4:")
        return XIEN_KIEU
